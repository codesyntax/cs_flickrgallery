...
class UpdatePhotosFromFlickr(BrowserView):
    ...
    def assemble_image_information(self, image):
        ...
        for size in sizes:
            srcset.append(
                f"{size.get('source')} {size.get('width')}w"
            )
    ...
    def flickr_username(self):
        return api.portal.get_registry_record("cs_flickrgallery.flickr_settings.flickr_user_id")
    ...
    def flickr_api_key(self):
        return api.portal.get_registry_record("cs_flickrgallery.flickr_settings.flickr_api_key")

    @property
    def flickr_api_secret(self):
        return api.portal.get_registry_record("cs_flickrgallery.flickr_settings.flickr_api_secret")
    ...
    def get_flickr_photoset_id(self, user_id):
        ...
        photosets = flickr.photosets.getList(user_id=user_id).get('photosets', {}).get('photoset', [])
    ...
    def gen_collection_sets(self, user_id, collection_id):
        ...
        try:
            for photoset in flickr.collections.getTree(
                    user_id=user_id, collection_id=collection_id).get('collections', {}).get('collection', []):
                yield photoset
        except:
            yield None
    ...
    def gen_photoset_photos(self, user_id, photoset_id):
        ...
        for photo in flickr.photosets.getPhotos(user_id=user_id, photoset_id=photoset_id, extras='date_upload', media='photos').get('photoset', {}).get('photo', []):
            yield photo
    ...
    def flickr(self):
        ...
        '''
        return flickrapi.FlickrAPI(self.flickr_api_key or "", self.flickr_api_secret or "", format="parsed-json")
